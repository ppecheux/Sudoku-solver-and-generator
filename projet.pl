...
remplirGrilleUtilisateur(G, Res):- 				remplirGrille2(G, G, Res).
...
remplir2(G, [0|G1], Res):- 	repeat, writeln('Ligne a remplir:'),
							readln([I|_]),
...
sudoku():- 	grilleVide(G),
			remplirGrille(G, G1),
			%listToSudoku(G1),
			writeln('Facile(0), difficile(1) ?'),
			readln([X]),
			N is 30+5*X,
			vidageGrille(G1, G2, N),
			listToSudoku(G2),
			remplir(G2, Res).	
